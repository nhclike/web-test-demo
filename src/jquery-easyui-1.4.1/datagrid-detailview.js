var detailview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	insertRow: function(target, index, row){
		...
		function _insert(frozen){
			...
			var newDetail = null;
			if (isAppend){
				newDetail = tr.next().clone();
				tr.insertAfter(tr.next());
			} else {
				newDetail = tr.next().next().clone();
			}
...
$.extend($.fn.datagrid.methods, {
	...
	isExpandedRow : function(jq, index){ //扩展By26007-20150608
		var expander = $(jq[0]).datagrid('getExpander', index);
		if(expander){
			return expander.hasClass('datagrid-row-expand');
		}
