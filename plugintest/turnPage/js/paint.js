/**
 * Created by lij on 2018/3/8.
 */
//标注功能
  //var o_res = null; // 承载数据 >> 承载ajax获取的数据;
var paint = {
    init: function() {
      this.load();
    ...
    load: function() {
      ...
      this.color = ["#000000", "#FF0000", "#80FF00", "#00FFFF", "#FF8000", "#408080", "#8000FF", "#CCCC00"]; //画笔颜色值
      this.fontWeight = [1, 2, 3];
      this.$ = function(id) { return typeof id == "string" ? document.getElementById(id) : id; };
      this.canvas = this.$("can"+canvasPage);
      this.canvas.style.cursor = 'crosshair';//画图状态鼠标指示
    ...
    bind: function() {
      ...
      this.canvas['on' + t.StartEvent] = function(e) {
        ...
        var left = $('.container')[0].offsetLeft + $('.shadow')[0].offsetLeft;
        var top = $('.container')[0].offsetTop + $('.shadow')[0].offsetTop+70;
        //console.log(left,top)
        var _x = touch.clientX - touch.target.offsetLeft-left; //鼠标在画布上的x坐标，以画布左上角为起点
        var _y = touch.clientY - touch.target.offsetTop-top; //鼠标在画布上的y坐标，以画布左上角为起点
        if(t.isEraser) {
    ...
    redraw: function() {
      var ctx = document.getElementById("can"+canvasPage).getContext('2d');
    ...
    getUrl: function() {
      ...
      var ctx = document.getElementById("can"+turnPage).getContext('2d');
      img = new Image();
